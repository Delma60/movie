...
export async function updateTitle(titleId: string, formData: FormData): Promise<void> {
  ...
  const editPath = `/admin/titles/${titleId}/edit`;
  ...
  if (!VALID_TYPES.includes(type)) {
    redirect(`${editPath}?error=invalid_type`);
  }
  if (!VALID_STATUSES.includes(status)) {
    redirect(`${editPath}?error=invalid_status`);
  }

  const slug = slugify(slugInput || title);
  if (!slug) {
    redirect(`${editPath}?error=invalid_slug`);
  }
  ...
  const [existingSlug] = await db
    ...
    .where(eq(titles.slug, slug))
    .limit(1);
  if (existingSlug && existingSlug.id !== titleId) {
  ...
  if (yearRaw) {
    year = Number(yearRaw);
    if (!Number.isFinite(year) || year < 1888 || year > 2100) {
      redirect(`${editPath}?error=invalid_year`);
    }
  }
  ...
  if (type === "movie" && durationRaw) {
    durationMinutes = Number(durationRaw);
    if (!Number.isFinite(durationMinutes) || durationMinutes <= 0) {
      redirect(`${editPath}?error=invalid_duration`);
    }
  }
