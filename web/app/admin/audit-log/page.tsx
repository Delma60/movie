...
import { getAdminAuditLog, AUDIT_ACTIONS, AUDIT_TARGET_TYPES } from "@/lib/admin-audit";
...
interface AdminAuditLogPageProps {
  searchParams: Promise<{ q?: string; action?: string; targetType?: string; page?: string }>;
}
...
export default async function AdminAuditLogPage({ searchParams }: AdminAuditLogPageProps) {
  ...
  const action = AUDIT_ACTIONS.includes(params.action as any) ? params.action : undefined;
  const targetType = AUDIT_TARGET_TYPES.includes(params.targetType ?? "") ? params.targetType : undefined;
    ...
    <main className="admin-page admin-audit-page">
        ...
        filters={[
          ...
          },
          {
            ...
            options: [{ value: "", label: "All target types" }].concat(
              AUDIT_TARGET_TYPES.map((t) => ({ value: t, label: t[0].toUpperCase() + t.slice(1) })),
            ),
          },
        ]}
        singularLabel="entry"
      ...
      <AdminTable
        ...
        rows={result.rows.map((entry) => (
          <tr key={entry.id}>
            <td className="admin-table-dim">{formatDateTime(entry.createdAt)}</td>
            <td>
              <span className="admin-table-primary">{entry.actorName}</span>
              <span className="admin-table-sub">{entry.actorEmail}</span>
            </td>
            <td>{ACTION_LABELS[entry.action] ?? entry.action}</td>
            <td className="admin-table-dim">{entry.targetLabel ?? entry.targetId ?? "—"}</td>
            <td className="admin-table-dim">{formatMetadata(entry.metadata as any) ?? "—"}</td>
          </tr>
        ))}
